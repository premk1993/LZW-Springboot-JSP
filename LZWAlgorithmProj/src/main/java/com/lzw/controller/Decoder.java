...
public class Decoder {
	...
	public String decoder(String decodedString) throws IOException {
		
		List<String> inputCodes = Arrays.asList(decodedString.split("\\s*,\\s*"));		
		double maxTblSize=Math.pow(2, bitLength);
		String prevString;
		String currString;
		prevString=dcdTABLE.get(Integer.parseInt(inputCodes.get(0)));
		...
		return dcdOutputString.toString();
	}
	 ...
	 * */
	public String generateDecodedFile(String decodedString) throws IOException {
		Timestamp timestamp = new Timestamp(System.currentTimeMillis());		
		String decodedFilePath= inputFilePath.concat("/"+Long.toString(timestamp.getTime()).concat("_Decoded.txt"));
